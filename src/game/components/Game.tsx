...
export function Game({ onBackToMenu }: GameProps) {
  ...
  const [esFallo, setEsFallo] = useState(false); // para saber si el modal es por fallo
  ...
  const narrativaCorta = fase.narrativa.split(/[.!?]/).map(s => s.trim()).find(Boolean) || fase.narrativa;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setTiempoRestante(prev => Math.max(0, prev - 100)), 100);
  ...
  const handleOptionSelect = useCallback((opcion: Opcion) => {
    ...
    if (opcion.correcta) {
      ...
      // Si la opción tiene primerPlanoFallo, mostramos modal antes del Game Over
      if (opcion.primerPlanoFallo) {
        setMostrarPrimerPlano(true);
        setInfoPrimerPlano(opcion.primerPlanoFallo);
        setEsFallo(true);
      } else {
        // Si no tiene, esperar 2 segundos y pasar a derrota
        setTimeout(() => {
          setAnimacionFallo(false);
          setEstado('derrota');
        }, 2000);
  ...
  const cerrarPrimerPlano = () => {
    ...
    setAnimacionFallo(false);
    if (esFallo) {
      // Si era un fallo, después del modal vamos a derrota
      setEstado('derrota');
    } else {
      // Era acierto: avanzar fase o victoria
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-b from-green-800 to-green-900">
      ...
      <div className="relative flex-[3] bg-cover bg-center" style={{ backgroundImage: 'url(/sprites/campo_futbol.png)' }}>
        ...
        {!mostrarPrimerPlano && estado !== 'victoria' && estado !== 'derrota' && fase.rival && (
          <div className="absolute left-[15%] bottom-[20%] z-10">
            ...
            />
            <p className="text-center text-white text-xs mt-1 bg-red-900/70 px-2 py-0.5 rounded" style={{ fontFamily: '"Press Start 2P", monospace' }}>{fase.rival.nombre}</p>
          </div>
        )}
        <div className="absolute right-[15%] bottom-[20%] z-10">
          <img
            ...
            onError={(e) => { if (!e.currentTarget.src.includes('maradona_izq.png')) e.currentTarget.src = '/sprites/maradona_izq.png'; }}
            className={`w-28 h-28 md:w-40 md:h-40 object-contain transition-all duration-300 ${estado === 'decision' && !mostrarPrimerPlano ? 'animate-pulse' : ''} ${animacionExito ? 'scale-110' : ''}`}
            style={{ filter: animacionExito ? 'drop-shadow(0 0 20px #00ff00)' : animacionFallo ? 'grayscale(0.7) brightness(0.6)' : 'none' }}
          />
          <p className="text-center text-white text-xs mt-1 bg-blue-600/70 px-2 py-0.5 rounded" style={{ fontFamily: '"Press Start 2P", monospace' }}>{gol.autor.apodo}</p>
        </div>
        <div className="absolute top-4 left-4 bg-black/60 text-white text-xs px-2 py-1 rounded" style={{ fontFamily: '"Press Start 2P", monospace' }}>Fase {faseActual + 1} / {gol.fases.length}</div>
        <div className="absolute top-4 right-4 bg-black/60 text-white text-xs px-2 py-1 rounded" style={{ fontFamily: '"Press Start 2P", monospace' }}>{gol.autor.nombre}</div>
      </div>
      ...
      <div className="flex-1 bg-black/80 border-t-4 border-yellow-600 shadow-2xl px-4 py-3 flex flex-col justify-center">
        {estado === 'decision' && !mostrarPrimerPlano && (
          <div className="w-full max-w-4xl mx-auto space-y-3">
            <h3 className="text-yellow-400 text-sm md:text-base text-center" style={{ fontFamily: '"Press Start 2P", monospace' }}>{fase.titulo}</h3>
            <div className="bg-gray-900/80 border border-gray-600 rounded p-2">
              <p className="text-white text-xs md:text-sm leading-relaxed text-center" style={{ fontFamily: '"Press Start 2P", monospace' }}>{narrativaCorta}</p>
            </div>
        ...
        {estado === 'resultado' && !mostrarPrimerPlano && (
          <div className="w-full max-w-2xl mx-auto">
            <div className="bg-black/90 border-4 border-white rounded-lg p-4">
              <h3 className={`text-lg mb-3 text-center ${opcionSeleccionada?.correcta ? 'text-green-400' : 'text-red-400'}`} style={{ fontFamily: '"Press Start 2P", monospace', textShadow: '2px 2px 0px #000' }}>
                {opcionSeleccionada?.correcta ? '¡BIEN HECHO!' : '¡FALLASTE!'}
              </h3>
              <p className="text-white text-sm leading-relaxed text-center" style={{ fontFamily: '"Press Start 2P", monospace', lineHeight: '1.5', textShadow: '2px 2px 0px #000' }}>{mensajeResultado}</p>
              {mensajeDialogo && (
                <p className="mt-3 text-center text-cyan-300 text-xs border-t-2 border-cyan-300/40 pt-2 italic" style={{ fontFamily: '"Press Start 2P", monospace', lineHeight: '1.5', textShadow: '2px 2px 0px #000' }}>“{mensajeDialogo}”</p>
              )}
            </div>
          </div>
        )}
      ...
      {/* Modal (primer plano) tanto para acierto como para fallo */}
      {mostrarPrimerPlano && infoPrimerPlano && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90">
          <div className="bg-black border-4 border-yellow-500 rounded-lg p-6 text-center max-w-md w-full mx-4">
            ...
            />
            <p className="text-yellow-300 text-lg mb-2" style={{ fontFamily: '"Press Start 2P", monospace' }}>{infoPrimerPlano.personaje}</p>
            <p className="text-white text-sm italic" style={{ fontFamily: '"Press Start 2P", monospace' }}>“{infoPrimerPlano.dialogo}”</p>
            <button onClick={cerrarPrimerPlano} className="mt-4 px-4 py-2 bg-yellow-600 text-black rounded text-xs hover:bg-yellow-500 transition-colors" style={{ fontFamily: '"Press Start 2P", monospace' }}>CONTINUAR</button>
          </div>
        </div>
      )}
